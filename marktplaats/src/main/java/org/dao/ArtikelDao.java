...
public class ArtikelDao extends Dao<Artikel, Long> {
    ...
    public List<Artikel> zoekInAlleBeschikbare(String zoekterm){
        return em.createNamedQuery(printKlasseNaam() + ".zoekInAlleBeschikbare", T()).setParameter(1, "%".concat(zoekterm).concat("%")).setParameter(2,"%".concat(zoekterm).concat("%")). getResultList();
    }
