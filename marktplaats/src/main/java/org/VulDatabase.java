...
public class VulDatabase {
    ...
    public static void start(CategorieDao catDao, ArtikelDao artDao, GebruikerDao gebDao) {
            ...
            Artikel skippybal = Product.builder().bezorgwijze(Arrays.asList(AFHALEN, VERZENDEN, MAGAZIJN, REMBOURS)).naam("Skippybal").beschrijving("Hier kun je heerlijk op bouncen").prijs(new BigDecimal("9.50")).categorie(bal).aanbieder(piet).build();
            Artikel stuiterbal = Product.builder().bezorgwijze(Arrays.asList(VERZENDEN)).naam("Stuiterbal").beschrijving("Deze bal houdt nooit meer op met stuiteren. Iedereen wordt gek!").prijs(new BigDecimal("0.50")).categorie(bal).aanbieder(piet).build();
            Artikel vwBeetle = Product.builder().bezorgwijze(Arrays.asList(AFHALEN, MAGAZIJN)).naam("Volkwagen Beetle").beschrijving("Prachtig kevertje, maakt lawaai als je het beweegt. Excl. batterijen").prijs(new BigDecimal("17.95")).categorie(minAuto).aanbieder(piet).build();
            Artikel ferrari = Product.builder().bezorgwijze(Arrays.asList(MAGAZIJN)).naam("Ferrari F50").beschrijving("Voor de jonge patsers. Dikke pooierbak").prijs(new BigDecimal("89.00")).categorie(minAuto).aanbieder(arie).build();
            ...
            nellie.plaatsInWinkelwagen(ferrari);
